class AchievementsController < ApplicationController
  ...
  def index
    @tokushima_temples = fetch_temple_photos(Temple.where(prefectures: '徳島県').where('distance_from_start <= ?', user_total_distance))
    @kochi_temples = fetch_temple_photos(Temple.where(prefectures: '高知県').where('distance_from_start <= ?', user_total_distance))
    @ehime_temples = fetch_temple_photos(Temple.where(prefectures: '愛媛県').where('distance_from_start <= ?', user_total_distance))
    @kagawa_temples = fetch_temple_photos(Temple.where(prefectures: '香川県').where('distance_from_start <= ?', user_total_distance))
  end
  ...
  def fetch_place_id(name)
    ...
    response = HTTParty.get(url, query: { input: name, inputtype: 'textquery', fields: 'place_id', key: ENV['MAPS_API_KEY'] })
    if response.success?
